...
class Query {
   ...
   * or UPDATE query (where supported).
   *
  ...
  inheritAlias(parent) {
    this.config.index = ++parent.config.index;
    this.config.alias = `${this.config.alias}_${this.config.index}`;
   ...
   * model instance. Note that the `from` option defaults to {@link Model.table}
   * for the model that's passed to {@link Query}'s constructor.
   *
   ...
   * work. If that's not the case then you could use the {@link Query#fields}
   ...
   * ```
   *
   * @param {Query|Model|SqlBricks|string} from The value to use for the FROM
   * clause. If passed as a {@link Model} class (note - note a Model instance),
   * then a {@link Query} instance is constructed from it via
   * {@link Model.query}.
   *
   * @param {object} [options] If the `from` parameter is a {@link Model} class
   * or a {@link Query} instance, these options are passed to
   * {@link Query#setOptions}.
   *
   * @returns {Query} The same {@link Query} instance to allow chaining.
   *
   * @todo support multiple `from` clauses, both as an array and via multiple
   * invocations
   */
  from(from, options) {
    const fromByAlias = this.options.from || {};
    ...
    from.forEach(from => {
      if (from.prototype instanceof Model) {
        from = from.query;
      }
      if (from instanceof Query) {
        from.config.isFrom = true;
        from.inheritAlias(this).setOptions(options);
        fromByAlias[from.config.alias] = from;
      } else if (from instanceof this.sql || isString(from)) {
        fromByAlias[this.config.alias] = from;
      } else if (isObject(from)) {
        Object.entries(from).forEach(([alias, value]) => {
          if (value.prototype instanceof Model) {
            value = value.query;
          }
          if (value instanceof Query) {
            value.config.isFrom = true;
            value.config.alias = alias;
            value.setOptions(options);
          }
          fromByAlias[alias] = value;
        });
      }
    });

    return this.setOption('from', fromByAlias);
  }
  ...
  // TODO: remove `formatTable` here i.e. remove the `schema` from the alias
  ...
  getColumns(fields) {
    return Object.entries(fields).reduce((aliased, [alias, field]) => {
      ...
      const formattedAlias = this.formatFieldAlias(alias);
      const formattedColumn = this.config.isFrom
        ? column
        : `${column} as ${formattedAlias}`;

      aliased.push(this.sql(formattedColumn));
  ...
  async prepareInsert(data, options) {
    ...
    return Promise.all(
      batches.map(async batch => {
        const sql = this.sql.insert(table, batch).values();
  ...
  async getFrom() {
    ...
    return Promise.all(
      Object.entries(from).map(async ([alias, value]) => {
        alias = this.quote(alias);

        if (value instanceof Query) {
          const sql = await value.prepareFetch();
          return sql.as(alias);
        }

        return this.sql(`${this.quote(value)} as ${alias}`);
      })
    );
  }
