...
class Query {
  ...
  async getFrom() {
    ...
    if (from instanceof Query) {
      // TODO: abstract this into a function (also used by @knorm/relations)
      from.config.index = ++this.config.index;
      from.config.alias = `${from.config.alias}_${from.config.index}`;
      from.config.isFrom = true;

      const sql = await from.prepareFetch();

      return sql.as(alias);
    }

    if (from instanceof this.sql) {
      from = from.toString();
    }

    return this.sql(`${this.quote(from)} as ${alias}`);
  }
